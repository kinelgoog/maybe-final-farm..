...
function startFarming(accountId) {
  ...
  client.on('loggedOn', () => {
    ...
    if (!account.steamId && client.steamID) {
...
function trackCardDrops(accountId) {
  ...
  setInterval(() => {
    ...
    if (expectedDrops > account.totalCards) {
      ...
      if (newDrops > 0) {
        notifyClient(accountId, `🎴 Получено ${newDrops} новых карточек!`);
      }
    }
  }, 300000);
}
  ...
  https.get(url, (res) => {
    ...
    res.on('end', () => {
      try {
        ...
        console.error('Error parsing games response:', e);
      }
    });
  }).on('error', (err) => {
...
function processGamesList(accountId, games) {
  ...
  if (gamesToFarm.length > 0) {
    ...
    notifyClient(accountId, `🎮 Добавлено ${gamesToFarm.length} новых игр для фарминга!`);
  }
}
...
function checkAchievements(accountId, totalCards) {
  ...
  thresholds.forEach(t => {
    if (totalCards >= t.cards && !account.achievements.includes(t.id)) {
      ...
      notifyClient(accountId, `🏆 Достижение: ${t.name}`);
    }
  });
}

function notifyClient(accountId, message) {
  // В реальном приложении можно использовать WebSocket или SSE
...
app.post('/accounts', (req, res) => {
  ...
  if (validApps.length === 0) {
    // Используем стандартные игры, если не указаны
    validApps.push(730, 322170); // CS2, Geometry Dash
    // Можно добавить CS:GO если нужно: 4465480
  }
  ...
  if (action === 'start') {
    ...
    if (clients[account.id]) {
      ...
      if (account.startTime) {
        const sessionTime = Date.now() - account.startTime;
        account.totalMinutes += Math.floor(sessionTime / 60000);
...
(function() {
  ...
  function renderGames(appIds) {
    ...
    const gameNames = {
      ...
      '4465480': 'CS:GO', // Правильный ID
      '322170': 'Geometry Dash' // Правильный ID
    };
  ...
  el('modal-add').onclick = () => {
    ...
    const apps = appsStr ? appsStr.split(',').map(s => parseInt(s.trim())).filter(n => !isNaN(n)) : [730, 322170, 4465480];
